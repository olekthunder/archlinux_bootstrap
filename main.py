...
def setup_bootloader(i: archinstall.Installer, cfg: AppConfig):
    archinstall.SysCommand(
        f"/usr/bin/arch-chroot {i.target} bootctl --path=/boot install"
    )
...
def misc_install(stack: ExitStack, cfg: AppConfig):
    ...
    with open(f"{i.target}/etc/locale.conf", "w") as fh:
        for k, v in cfg.lc_conf_vars.items():
            fh.write(f"{k}={v}\n")
    archinstall.SysCommand(f"/usr/bin/arch-chroot {i.target} locale-gen")
    i.set_timezone(cfg.time_zone)
    i.activate_ntp()
    archinstall.SysCommand(f"/usr/bin/arch-chroot {i.target} hwclock --systohc")
    archinstall.SysCommand(f"/usr/bin/arch-chroot {i.target} chmod 700 /root")
